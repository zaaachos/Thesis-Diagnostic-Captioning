...
class ImageEncoder:
    ...
    def __init__(self, encoder, images_dir_path, weights='imagenet'):
        ...
        if encoder == 'densenet201':
            ...
            self.image_encoder = Model(inputs=model.input, outputs=model.get_layer('avg_pool').output)
            ...
            self.image_encoder = Model(inputs=model.input, outputs=model.output)

        else:
            print("You have to initialize a valid version of image encoder\n"
                  "Choices are: [densenet201, densenet121, densenet169, efficientnet5, efficientnet7, resnet50v2]")
            print("Exiting...")
            return
    ...
    def encode(self, _image, verbose=0):
        if self.get_preprocessor() == 'cotnet':
            ...
            if self.get_preprocessor() == 'densenet':
                ...
                preprocessed_image_array = inceptionresnet_preprocess(image_array)
